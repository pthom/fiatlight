...
from typing import Any, Callable, Generic, List, NewType, Tuple, Type, TypeAlias, TypeVar
...
def dataclass_with_gui_registration(**kwargs: Any) -> Callable[[type[_TDataclass]], type[_TDataclass]]:
    ...
    def actual_decorator(cls: type[_TDataclass]) -> type[_TDataclass]:
...
def register_base_model(base_model_type: type[pydantic.BaseModel], **fiat_attrs: Any) -> None:
...
def base_model_with_gui_registration(**kwargs: Any) -> Callable[[type[_TBaseModel]], type[_TBaseModel]]:
    """Decorator to register a pydantic BaseModel with its autogenerated GUI implementation."""

    def actual_decorator(cls: type[_TBaseModel]) -> type[_TBaseModel]:
