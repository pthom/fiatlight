...
def test_creation() -> None:
    a: AnyDataWithGui[int] = _any_type_class_name_to_gui("int", capture_current_scope(), NO_CUSTOM_ATTRIBUTES)
...
def test_named_data_with_gui_creation() -> None:
    x = to_data_with_gui(1, capture_current_scope(), NO_CUSTOM_ATTRIBUTES)
...
def test_named_data_with_gui_serialization() -> None:
    d = to_data_with_gui(1, capture_current_scope(), NO_CUSTOM_ATTRIBUTES)
...
def test_enum_serialization() -> None:
    ...
    a = to_data_with_gui(MyEnum.A, capture_current_scope(), NO_CUSTOM_ATTRIBUTES)
...
def test_pydantic_serialization() -> None:
    ...
    a = A(x=1, y="hello")
    a_gui = to_data_with_gui(a, current_scope, NO_CUSTOM_ATTRIBUTES)
    assert a_gui.value == a

    as_dict = a_gui.save_to_dict(a_gui.value)
    assert as_dict == {"type": "Pydantic", "value": {"x": 1, "y": "hello"}}

    a2_gui = to_type_with_gui(A, current_scope, NO_CUSTOM_ATTRIBUTES)
    ...
    b = B(a=a, z=3.14)
    b_gui = to_data_with_gui(b, current_scope, NO_CUSTOM_ATTRIBUTES)
    assert b_gui.value == b

    as_dict = b_gui.save_to_dict(b_gui.value)
    assert as_dict == {"type": "Pydantic", "value": {"a": {"x": 1, "y": "hello"}, "z": 3.14}}

    b2_gui = to_type_with_gui(B, current_scope, NO_CUSTOM_ATTRIBUTES)
...
def test_type_storage() -> None:
    ...
    oi = to_type_with_gui(OptionalInt, scope_storage, NO_CUSTOM_ATTRIBUTES)
    assert oi._type == OptionalInt  # type: ignore

    OptionalInt2 = int | None
    oi2 = to_type_with_gui(OptionalInt2, scope_storage, NO_CUSTOM_ATTRIBUTES)
    assert oi2._type == OptionalInt2  # type: ignore

    from fiatlight.fiat_kits.fiat_image import ImageU8_3, Image

    gi = to_type_with_gui(ImageU8_3, scope_storage, NO_CUSTOM_ATTRIBUTES)
